...
Главное правило безопасности: уникальные сложные пароли и двухфакторная аутентификация.
    ...
    def create_testing_tab(self):
        ...
        # DDoS Test
        ddos_test_tab = self.test_tabs.add("Load Test")
        self.create_ddos_test_ui(ddos_test_tab)
    ...
    def create_ddos_test_ui(self, parent):
        ...
        self.ddos_test_btn = ctk.CTkButton(
            parent,
            text="Start Load Test",
            command=self.start_ddos_test
        )
        self.ddos_test_btn.grid(row=2, column=1, pady=10)
    ...
    def start_ddos_test(self):
        if not self.check_legal():
            return
        
        target = self.ddos_url.get()
        threads = int(self.threads.get())
        
        self.log("Starting load test (educational purposes only!)")
        
        for _ in range(threads):
            thread = threading.Thread(target=self.send_requests, args=(target,))
            thread.start()
